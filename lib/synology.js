...
Synology.prototype.shutdown = function(callback) {
    ...
    that._login().then(function() {
        let apiUrl = (that.version >= 6) ? '/webapi/entry.cgi' : '/webapi/dsm/system.cgi';
...
Synology.prototype.getDiskTemp = function(callback) {
    ...
    that._login().then(function(res) {
        let apiUrl = (that.version >= 6) ? '/webapi/entry.cgi' : '/webapi/dsm/info.cgi';
        let options = {
            url: that.url + apiUrl,
            qs: {
    ...
    that._login().then(function() {
        ...
        request(options, function(err, res, body) {
            if (!err && res.statusCode == 200) {
                ...
                callback("An error occured while getting SystemTemp: " + json.error.code);
            }
        });
    }).catch(function(err) {
...
Synology.prototype.getCpuLoad = function(callback) {
    ...
    that._login().then(function() {
        ...
        let apiUrl = (that.version >= 6) ? '/webapi/entry.cgi' : '/webapi/dsm/system_loading.cgi';
        let options = {
            url: that.url + apiUrl,
            qs: {
...
Synology.prototype.getDiskUsage = function(callback) {
    ...
    that._login().then(function() {
        let apiUrl = (that.version >= 6) ? '/webapi/entry.cgi' : '/webapi/dsm/volume.cgi';
        let options = {
            url: that.url + apiUrl,
            qs: {
